...
也可以在运行时通过 [Debugx Console](#-debugx-console-日志控制台) 的 Runtime 面板 / 游戏内叠层以可视化方式调整这些开关。
...
> [!NOTE]
> 旧版本中 `DebugxConsole` 只是一个运行时开关控制面板。现在它升级为完整的日志查看器，原来的运行时开关与测试开关被收纳进窗口内的 [Runtime 运行时面板](#runtime-运行时面板)；旧的「屏幕绘制日志」功能已移除，由[游戏内运行时叠层](#游戏内运行时叠层)取代。
...
- **Runtime**：显示 / 隐藏下方的 [Runtime 运行时面板](#runtime-运行时面板)。
- **Debugx Only**：只显示 Debugx 打印的日志，隐藏 Uncategorized。
- **Members**：按成员过滤，可多选；含 `All` 与各成员 `[key] 签名`，以及 `Uncategorized`。
- **搜索框**：按文本实时过滤（编辑器版支持正则、大小写、搜索堆栈）。
- **Log / Warning / Error**：三个带**计数**的类型开关，点击切换对应级别是否显示（超过 999 显示 `999+`）。
- **View**：视图选项菜单——`Show Timestamp`（时间戳列）、`Stack: Script Only`（堆栈仅显示业务脚本帧，隐藏引擎与插件内部帧）。
- **语言**：中 / EN 切换。
...
### Runtime 运行时面板
点击工具栏 **Runtime** 展开。此面板收纳了旧控制面板的全部功能，便于在编辑器里边跑边调：
- **启用游戏内运行时 Console**：是否在下次进入 Play 时自建[游戏内运行时叠层](#游戏内运行时叠层)（默认关闭，仅影响**编辑器 Play 模式**；真机需在代码中启用，详见该节）。
- **运行时开关**（仅 Play 模式可用）：`EnableLog`（日志总开关）、`EnableLogMember`（成员日志总开关）、`Only Key`（仅打印指定 Key 成员，`0` 为关闭），以及每个成员的独立开关。这些会实时改写 `Debugx` 的运行时状态。
- **测试开关**：`Awake test log` / `Update test log`，用于快速确认 Debugx 是否正常工作。

![](Documents/console_editor_runtime_1.png)
<!-- 截图占位：编辑器 Console 展开 Runtime 面板（启用叠层开关 + 运行时开关 + 测试开关） -->
...
2. 已启用运行时叠层开关 `DebugxStaticData.RuntimeConsoleEnabled`（默认关闭）：
   - **编辑器 Play 模式**：在 `Debugx Console -> Runtime` 面板勾选**启用游戏内运行时 Console**（下次进入 Play 时生效）。
...
叠层功能与编辑器版基本一致（触屏适配）：`Clear`、`Copy`、`Collapse`、`Debugx Only`、`Members` 成员过滤、`Time` 时间戳、`Net` 网络标记（All / Server / Client 循环）、`Log / Warning / Error` 类型过滤与计数，以及搜索（命中子串高亮）。此外 **Source** 弹层提供与编辑器 Runtime 面板等价的运行时开关（`EnableLog` / `EnableLogMember` / `Only Key` / 各成员开关），直接改写真实打印行为。选中某条日志可在下方查看消息与堆栈文本（运行时不支持源码跳转）。  
