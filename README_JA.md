...
> 2. プラグインを更新した後、DebugxLogger クラスが生成されない場合は、メニューの Tools > Debugx > Regenerate DebugxLogger Class を使用して強制的に再生成してください。
> 3. プラグインは 2.3.0 以前のバージョンではフォルダ構造と UPM リンクが変更されたため、正常に更新できず、古いバージョンを削除してから再インストールする必要があります。
