...
## 💻 動作環境
- `Unity 2022.3` 以降（それより古いバージョンは未検証）。
...
これらのスイッチは、実行時に [Debugx Console コンソール](#-debugx-console-コンソール) の Editor パネル / ゲーム内オーバーレイから視覚的に調整することもできます。
...
> [!NOTE]
> 旧バージョンでは `DebugxConsole` は実行時スイッチのコントロールパネルにすぎませんでした。現在は完全なログビューアに進化し、従来の実行時スイッチとテストスイッチはウィンドウ内の展開可能な [Editor パネル](#editor-パネル) に収められ、旧「画面描画ログ」機能は削除され [ゲーム内実行時オーバーレイ](#ゲーム内実行時オーバーレイ) に置き換えられました。
...
- **Members**：メンバーでフィルター、複数選択可。`All`、各メンバーの `[key] 署名`、および `Admin` / `Unregistered` / `Uncategorized` の疑似メンバーを含む。
- **Editor**：下部の展開可能な [Editor パネル](#editor-パネル) を表示 / 非表示——ビューオプション、ゲーム内 Console スイッチ、実行時スイッチ、テストスイッチ、UI 言語はすべてここに収められています。
- **検索ボックス**：テキストでリアルタイムフィルター（部分文字列マッチ、大文字小文字を区別しない）。
- **Log / Warning / Error**：ツールバー右端にある**カウント**付きの 3 つのタイプスイッチ。クリックで該当レベルの表示を切り替え（999 を超えると `999+` と表示）。
...
### Editor パネル
ツールバーの **Editor** をクリックして展開します。このパネルは、ツールバーから移したビューオプションと、旧コントロールパネルの実行時 / テストスイッチを一か所にまとめており、エディターで動かしながら調整するのに便利です：
- **ビューオプション**（常に利用可）：`Debugx Only`（`[Debugx]` タグ付きのログのみ表示し、Uncategorized を隠す）、`Show Timestamp`（各行の時刻列）、`Stack: Script Only`（詳細スタックで業務スクリプトフレームのみ表示し、エンジンとプラグイン内部フレームを隠す）。
- **ゲーム内 Console を有効化**：次回 Play 開始時に [ゲーム内実行時オーバーレイ](#ゲーム内実行時オーバーレイ) を自動生成するか（既定でオフ、**エディター Play モードのみ**に影響。実機ではコードで有効化が必要、当該セクション参照）。
- **実行時スイッチ**（Play モードのみ利用可）：`EnableLog`（ログのメインスイッチ）、`EnableLogMember`（メンバーログのメインスイッチ）、`Only this key`（指定 Key のメンバーのみ出力、`0` でオフ）。これらは `Debugx` の実行時状態をリアルタイムに書き換えます。（各メンバーの個別スイッチは `Preferences -> Debugx` またはゲームコードで設定してください。）
- **テストスイッチ**：`Awake test log` / `Update test log`。Debugx が正常に動作しているか素早く確認するため。
- **UI 言語**：中 / EN の切り替え。

![](Documents/console_editor_2.png)
<!-- スクリーンショット占位：エディター Console で Editor パネルを展開（ビューオプション + オーバーレイ有効化スイッチ + 実行時スイッチ + テストスイッチ + 言語） -->
...
2. 実行時オーバーレイスイッチ `DebugxStaticData.RuntimeConsoleEnabled` が有効（既定でオフ）：
   - **エディター Play モード**：`Debugx Console -> Editor` パネルで**ゲーム内 Console を有効化**にチェック（次回 Play 開始時に反映）。
